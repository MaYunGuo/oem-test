...
public class QuartzShipDataJob extends QuartzJobBean {
    ...
    public void executeInternal(JobExecutionContext jobExecutionContext) throws JobExecutionException {
        ...
        logUtils = new LogUtils(QuartzShipDataJob.class);
        AppContext.setCurrEventNumber(GUIDGenerator.javaGUID());
        AppContext.setCurrServiceName(QuartzShipDataJob.class.getSimpleName());


        JobDataMap dataMap = jobExecutionContext.getJobDetail().getJobDataMap();
        String task_name = (String) dataMap.get("task_name");
        String task_path = FTP_PATH  + File.separator + task_name + File.separator + "EXCEL" + File.separator + "SHIP";
        long startTimes = System.currentTimeMillis();
        logUtils.info(task_name + "出货数据解析开始执行---------------------------");
        try {
            ...
            if(filePath.exists() && filePath.isDirectory()){
                ...
                for(File shipFile : allFiles){
                    if(!shipFile.isFile()){
                        continue;
                    }
                    realPath = shipFile.getAbsolutePath();
